...
* Verwenden der Betriebssysteme Linux® und Microsoft® Windows®,
...
* Eine Instanz, die AEM 6.4, AEM 6.5 oder AEM Forms als Autoreninstanz für Cloud Service mit dem neuesten AEM Service Pack oder den neuesten Updates ausgeführt wird.
...
1. [Herunterladen und Installieren von AEM 6.4, AEM 6.5 oder an Bord von AEM Forms als Cloud Service](#aemquickstart)
...
### Laden Sie AEM 6.4 oder AEM 6.5 oder an Bord von AEM Forms als Cloud Service herunter und installieren Sie sie.{#aemquickstart}


Der Service für die automatisierte Formularkonvertierung wird auf der AEM-Autoreninstanz ausgeführt. Sie benötigen AEM 6.4, AEM 6.5 oder AEM Forms als Cloud Service, um eine AEM Autoreninstanz einzurichten.

* Wenn Sie AEM 6.4 oder AEM 6.5 nicht ausgeführt haben, laden Sie es von den folgenden Speicherorten herunter. Anweisungen zum Einrichten einer AEM- Autoreninstanz finden Sie nach dem Herunterladen von AEM unter [Bereitstellen und Verwalten](https://helpx.adobe.com/de/experience-manager/6-5/sites/deploying/using/deploy.html#defaultlocalinstall).:
...
* Wenn Sie AEM Forms als Cloud Service verwenden, lesen Sie an Bord nach [AEM Forms als Cloud Service](https://experienceleague.adobe.com/docs/experience-manager-forms-cloud-service/forms/setup-environment/setup-forms-cloud-service.html?lang=en#setup-environment) und [richten Sie eine lokale Entwicklungs-Umgebung](https://experienceleague.adobe.com/docs/experience-manager-forms-cloud-service/forms/setup-environment/setup-local-development-environment.html?lang=en#setup-environment) ein.

### (Nur für AEM 6.4 und AEM 6.5) Laden Sie AEM neueste Service Pack {#servicepack} herunter und installieren Sie es
...
### (Nur für AEM 6.4 und AEM 6.5) Laden Sie das AEM Forms-Add-On-Paket herunter und installieren Sie es. {#downloadaemformsaddon}
...
Wenn Sie AEM 6.4 oder AEM 6.5 im [Produktionsmodus](https://helpx.adobe.com/de/experience-manager/6-5/sites/administering/using/production-ready.html) (nicht im Ausführungsmodus zum Inhalt) ausführen, werden die Referenzpakete nicht installiert. Die Referenzpakete enthalten Beispieldesigns und -vorlagen. Für die Konvertierung eines PDF-Formulars in ein adaptives Formular ist mindestens ein Design und eine Vorlage für den automated forms conversion-Dienst erforderlich. Erstellen Sie ein eigenes benutzerdefiniertes Design und eine eigene Vorlage und zeigen Sie auf [Service-Konfiguration](#configure-the-cloud-service), um benutzerdefinierte Vorlagen und Designs zu verwenden, bevor Sie den Service verwenden.

Sie können das [AEM Forms Reference Assets](https://experience.adobe.com/#/downloads/content/software-distribution/en/aemcloud.html)-Paket auch auf Ihrer Autoreninstanz herunterladen und installieren. Es werden einige Referenz-Themen und Vorlagen erstellt.
...
* **Administratoren**: Administratoren sind für die Verwaltung der Adobe-Software und -Services für ihre Organisation verantwortlich. Administratoren gewähren Entwicklern in ihrer Organisation Zugriff zur Herstellung einer Verbindung zum Service für die automatisierte Formularkonvertierung, der in Adobe Cloud ausgeführt wird. Wenn ein Administrator für eine Organisation bereitgestellt wird, erhält der Administrator eine E-Mail mit dem Titel **[!UICONTROL Sie haben jetzt Administratorrechte zur Verwaltung von Adobe-Software und -Services für Ihre Organisation]**. Wenn Sie Administrator sind, suchen Sie in Ihrem Postfach nach E-Mail mit dem zuvor erwähnten Titel und fahren Sie mit [Gewähren Sie Zugriff auf Entwickler Ihres Unternehmens](#adduseranddevs) fort.

![E-Mail mit Administratorzugriffsberechtigung](assets/admin-console-adobe-io-access-grantedx75.png)

* **Entwickler**: Ein Entwickler verbindet eine lokale AEM Forms-Autoreninstanz mit dem Service für die automatisierte Formularkonvertierung, der in Adobe Cloud ausgeführt wird. Wenn ein Administrator einem Entwickler Rechte zum Herstellen einer Verbindung zum Service für die automatisierte Formularkonvertierung gewährt, wird eine E-Mail mit dem Titel „Sie haben jetzt Entwicklerzugriff zum Verwalten der Adobe API-Integrationen für Ihr Unternehmen“ an den Entwickler gesendet. Wenn Sie Entwickler sind, suchen Sie in Ihrem Postfach nach E-Mail mit dem zuvor erwähnten Titel und gehen Sie zu [Verbinden Sie Ihre lokale AEM mit dem Automated forms conversion-Dienst in der Adobe Cloud.](#connectafcadobeio)

![E-Mail zum Zuschuss für Entwickler](assets/email-developer-accessx94.png)

### (Nur für Administratoren von AEM 6.4 und AEM 6.5) Gewähren Sie Zugriff auf Entwickler Ihrer Organisation {#adduseranddevs}
...
* Melden Sie für AEM Forms als Cloud Service ein Support-Ticket an, um den E-Mail-Dienst](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/developing/development-guidelines.html?lang=en#sending-email) zu aktivieren.[
...
#### (Nur für AEM 6.4 und AEM 6.5) Öffentliche Zertifikate {#obtainpubliccertificates}
...
1. Tippen Sie auf **[!UICONTROL Öffentlichen Schlüssel herunterladen]** und speichern Sie die Zertifikatdatei *AEM-Adobe-IMS.crt* auf Ihrem Computer. Die Zertifikatdatei wird verwendet, um die Dienst-APIs in der Adobe Developer Console](#createintegration) zu konfigurieren. [ Tippen Sie auf **[!UICONTROL Weiter]**.
...
#### (Nur für AEM 6.4 und AEM 6.5) Konfigurieren Sie die Dienst-APIs in der Adobe Developer Console {#createintegration}
...
Erstellen Sie eine Cloud Service-Konfiguration, um Ihre AEM mit dem Konvertierungsdienst zu verbinden. Außerdem können Sie eine Vorlage, ein Design und Formularfragmente für eine Konvertierung angeben. Sie können mehrere Cloud-Service-Konfigurationen für jeden Formularsatz separat erstellen. Beispielsweise können Sie separate Konfigurationen für Formulare der Verkaufsabteilung und für Formulare für den Kundensupport erstellen. Führen Sie die folgenden Schritte aus, um ein eine Cloud-Service-Konfiguration zu erstellen:
...
1. Geben Sie auf der Registerkarte **[!UICONTROL Erweitert]** der Seite **[!UICONTROL Konfiguration für die automatisierte Formularkonvertierung erstellen]** Werte für die folgenden Felder ein:
   ...
   <td>(Nur für AEM 6.4 und AEM 6.5) Wählen Sie die Option, um Adobe Analytics für alle konvertierten Formulare zu aktivieren. Bevor Sie diese Option verwenden, stellen Sie sicher, dass Adobe Analytics für Ihre AEM Forms-Instanz aktiviert ist.</td>
   </tr>
   </tbody>
   </table>
